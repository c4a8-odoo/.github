...
- If Docker is unavailable in the environment but podman is available, use `alias docker='podman'`. 
  Otherwise stop and report the blocker instead of switching to ad-hoc raw `odoo-bin` execution.
